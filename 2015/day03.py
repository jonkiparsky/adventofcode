...
house at his starting/ending location. ^v^v^v^v^v delivers a bunch of presents
...
def solve(input):
    ...
    houses[loc] += 1    
    for i in input:
        loc = tuple([sum(tup) for tup in zip(loc, move[i])])
        houses[loc] += 1        
    return sum([1 for v in houses.values() if v >= 1])
...
#        print ix, i, locs
        loc = locs[ix % 2]
        locs[ix % 2] = tuple([sum(tup) for tup in zip(loc, move[i])])
        houses[locs[ix % 2]] += 1        
    return sum([1 for v in houses.values() if v >= 1])
