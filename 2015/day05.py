...
def nice1(candidate):
    if any ([s in candidate for s in ["ab", "cd", "pq", "xy"]]):
        print "1"
        return False
    if not any ([t1 == t2 for (t1, t2) in zip(candidate, candidate[1:])]):
        print "2"
        return False
    if sum([candidate.count(vowel) for vowel in "aeiou"]) < 3:
        print "3"
        return False
    return True


pat1 = re.compile(r"(.)(.).*\1\2")
pat2 = re.compile(r"(.).\1")

def nice2(candidate):
    return pat1.search(candidate) and pat2.search(candidate)
